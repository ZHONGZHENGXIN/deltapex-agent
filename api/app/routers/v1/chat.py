...
from app.crud.chat import create_chat, get_all_chats, get_chat, soft_delete_chat, update_chat
...
async def create_chat_message_api(
    ...
    history = get_all_messages(message_in.chat_id, session)
    messages = [MessageOut.model_validate(message) for message in history]
    ...
    current_turns = get_chat_turn_count(session, message_in.chat_id)
    ...
    messages.append(MessageOut.model_validate(user_message))
    ...
    if not stream:
        ...
        try:
            ...
            MessageLogger.log_ai_response_error(str(exc), message_in.chat_id)
            assistant_message = create_message(
                MessageCreate(
                    chat_id=message_in.chat_id,
    ...
    async def stream_gen():
        ...
        try:
            ...
            async for chunk, usage_info in create_agent_response_stream(messages, agent, user_id, chat.id, None):
                if chunk:
                    content_acc += chunk
                    chunk_count += 1
                    yield chunk
            ...
            if message_id:
                with next(get_session()) as final_session:
                    ...
                    MembershipService(final_session).record_usage(user_id, message_in.chat_id, 1, total_tokens)
            ...
            if message_id:
                with next(get_session()) as error_session:
                    ...
                    MembershipService(error_session).record_usage(user_id, message_in.chat_id, 1, 0)
...
def update_chat_api(chat_id: int, chat_in: ChatUpdate, session: SessionDep, user: UserDep, lang: LangDep) -> ChatOut:
...
def get_chat_api(chat_id: int, session: SessionDep, user: UserDep, lang: LangDep) -> ChatOut:
    ...
    chat_out.agent = AgentSchema.model_validate(agent)
    chat_out.messages = [MessageOut.model_validate(message) for message in messages]
    return chat_out
