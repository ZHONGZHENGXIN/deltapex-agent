...
export const metadata = {
  ...
  metadataBase: new URL('https://build-ai-template.v2ai.cn'),
  icons: '/img/logo_square_tight.png',
} satisfies Metadata

const repo = 'https://github.com/open-v2ai/build-ai-template'

const CustomBanner = async ({ lang }: I18nLangAsyncProps) => {
  const { t } = await useServerLocale(lang)
  return (
    <Banner
      storageKey="bait-banner"
    >
    ...
    <Navbar
      logo={(
        <>
          <img
            className="mr-2"
            src="/img/logo_square_tight.png"
            alt="Logo"
            width={32}
            height={32}
          />
          <span className="text-xl font-bold relative text-foreground dark:text-white tracking-widest">
            { t('systemTitle') }
          </span>
        </>
      )}
...
export default async function RootLayout({ children, params }: LayoutProps<'/[lang]'>) {
  ...
  const title = 'Build AI Template'
  const description = 'A template for building AI applications'
    ...
    >
      ...
      <body>
        <ThemeProvider
          ...
          defaultTheme="system"
          enableSystem
          storageKey="bait-docs-theme-provider"
        ...
        >
          <Layout
            ...
            editLink={null}
            docsRepositoryBase="https://github.com/open-v2ai/build-ai-template"
            footer={(
